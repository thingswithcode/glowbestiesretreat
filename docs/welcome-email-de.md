# Willkommens-Mail (Deutsch) für Mailchimp / Gmail

Diese E-Mail geht raus, sobald sich jemand über das Formular anmeldet.
Fertige HTML-Versionen liegen daneben:
- `welcome-email.html` (für Mailchimp, mit Merge-Tags wie `*|FNAME|*`)
- `welcome-email-gmail.html` (für manuelles Senden aus Gmail, mit `[Vorname]` zum selbst Ersetzen)

Aufbau: Logo, Gruppenfoto vom Retreat, dann: wer wir sind, was wir machen, warum wir es machen, was uns besonders macht (glutenfrei), Eindruck vom letzten Retreat, Instagram, Schluss.

---

## Betreffzeile (eine auswählen)
- Schön, dass du dabei bist
- Willkommen in unserer kleinen Community
- Schön, dass du da bist

## Preheader (Vorschautext)
Schön, dass du in unserer kleinen Community bist. Move. Nourish. Connect.

---

## Text

**Schön, dass du in unserer kleinen Community bist.**

Hallo [Vorname], vielen Dank für deine Anmeldung.

Wir sind Eli und Leonie, die zwei Gründerinnen von Glow Besties. Wir kommen beide aus schnellen, fordernden Corporate-Jobs und wissen selbst, wie schwer es ist, wirklich abzuschalten und runterzukommen. Genau deshalb haben wir Glow Besties ins Leben gerufen: Retreats für Frauen, denen es genauso geht.

Glow Besties steht für move, nourish, connect: für Frauen, die viel leisten und sich bewusst Zeit nehmen wollen, um wieder bei sich selbst anzukommen.

Was uns besonders macht: Bei uns ist alles komplett glutenfrei. Leonie lebt selbst mit Zöliakie und weiß, wie schwer es sein kann, überall unbeschwert essen zu können. Deshalb schaffen wir einen Ort, an dem einfach jede genießen kann, ohne Sorge und ohne Verzicht.

Unser erstes Retreat, The Alpine Reset, liegt hinter uns: drei Tage Bewegung, Natur, gutes Essen und echte Verbindung in den Bergen über dem Montafon. Schau dir an, wie es war.

**[ Unser letztes Retreat ansehen ]** (Button auf: https://glowbestiesretreats.com/experience.html)

Folg uns gern auf Instagram. Dort teilen wir Eindrücke von den Retreats, die nächsten Events, Rezepte und Behind-the-Scenes.

**[ Folge uns auf Instagram ]** (Button auf: https://www.instagram.com/theglowbesties/)

Wir stehen ganz am Anfang unserer Reise und freuen uns riesig, dass du dabei bist.

Alles Liebe,
Eli & Leonie
Glow Besties Retreats

---

### Bild in der Mail
- Banner (Gruppenfoto mit Logo als Ebene im Korall-Ton): https://glowbestiesretreats.com/images/experience/hero-email.jpg
- Hinweis: Dieses Banner-Bild ist neu und muss beim naechsten Website-Deploy mit hochgeladen werden, damit es in der Mail laedt. Bis dahin kann man es lokal unter images/experience/hero-email.jpg ansehen.
